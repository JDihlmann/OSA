...
class Scan():
    def __init__(self, mesh, camera_transform, resolution=400, calculate_normals=True, fov=1, z_near=0.1, z_far=10):
        ...
        color, depth, normal = render_normal_and_depth_buffers(mesh, camera, self.camera_transform, resolution)

        self.normal_buffer = normal if calculate_normals else None
        self.color_buffer = color if calculate_normals else None
        ...
        if calculate_normals:
            colors = color[indices[:, 0], indices[:, 1]]
            normals = normal[indices[:, 0], indices[:, 1]] / 255 * 2 - 1
