...
class SurfacePointCloud:
    ...
    def get_sdf(self, query_points, use_depth_buffer=False, sample_count=11, return_gradients=False):
        if use_depth_buffer:
            ...
            if return_gradients:
                colors = self.colors[indices[:, 0]]
                gradients = self.normals[indices[:, 0]]
        ...
        if return_gradients:
            ...
            return distances, gradients, colors
        else:
            return distances
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000, return_gradients=False):
        ...
        if return_gradients:
            ...
            colors = np.concatenate([batch[2] for batch in batches])
            return distances, gradients, colors
        else:
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=20, min_size=0, return_gradients=False, near_points=True):
        query_points = []
        if( near_points ):
            ...
            surface_points = self.get_random_surface_points(surface_sample_count, use_scans=use_scans)
            distances = np.expand_dims(np.linalg.norm(surface_points, axis=1), axis=1)
            max_distance_offset = 1.0 - distances
            query_points.append(surface_points + surface_points / distances * max_distance_offset * np.random.uniform(size=(surface_sample_count, 1)) * 0.1) 

            unit_sphere_sample_count = number_of_points // 2
        ...
        if return_gradients:
            return query_points, sdf, gradients, colors
        else:
...
def create_from_scans(mesh, bounding_radius=1, scan_count=100, scan_resolution=400, calculate_normals=True):
    ...
    return SurfacePointCloud(mesh, 
        ...
        colors=np.concatenate([scan.colors for scan in scans], axis=0) if calculate_normals else None,
        scans=scans
    )
